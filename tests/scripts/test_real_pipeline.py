#!/usr/bin/env python
"""
Real Model Pipeline Test - Qwen3-VL-4B-Instruct + Florence-2
...
def display_reasoning_results(steps: List[ReasoningStep], cot_text: Optional[str] = None):
    """Display reasoning stage results in a formatted table."""
    if cot_text:
        console.print("\n[bold cyan]Chain of Thought:[/bold cyan]")
        # Show first 500 chars
        preview = cot_text[:500]
        if len(cot_text) > 500:
            preview += "..."
...
def validate_and_log_coordinates(result: PipelineResult, image: Image.Image):
    ...
    if result.key_evidence:
        ...
        for kev in result.key_evidence[:3]:
            ...
            table.add_row(
                f"({bbox_norm[0]:.3f}, {bbox_norm[1]:.3f}, {bbox_norm[2]:.3f}, {bbox_norm[3]:.3f})",
                kev.description[:50] + "..." if len(kev.description) > 50 else kev.description,
                status
            )
        console.print(table)
...
def display_performance_summary(timings: Dict[str, float], memory_mb: Optional[float] = None):
...
def validate_all_bboxes(result: PipelineResult) -> Dict[str, Any]:
    ...
    for i, ev in enumerate(result.evidence):
        ...
        if not all(0 <= coord <= 1 for coord in bbox):
            ...
            validation["issues"].append(
                f"Evidence {i} bbox out of range: {bbox}"
            )
        ...
        if bbox[0] >= bbox[2] or bbox[1] >= bbox[3]:
            ...
            validation["all_valid"] = False
            validation["issues"].append(
                f"Evidence {i} bbox invalid ordering: {bbox}"
            )
        ...
        if any(math.isnan(c) or math.isinf(c) for c in bbox):
            ...
            validation["all_valid"] = False
            validation["issues"].append(
                f"Evidence {i} bbox has NaN/Inf: {bbox}"
            )
    ...
    for i, ev in enumerate(result.key_evidence):
        ...
        if not all(0 <= coord <= 1 for coord in bbox):
            ...
            validation["all_valid"] = False
            validation["issues"].append(
                f"Key evidence {i} bbox out of range: {bbox}"
            )
        ...
        if any(math.isnan(c) or math.isinf(c) for c in bbox):
            ...
            validation["all_valid"] = False
            validation["issues"].append(
                f"Key evidence {i} bbox has NaN/Inf: {bbox}"
            )
    
    return validation


def visualize_bboxes(image: Image.Image, evidences: List[GroundedEvidence], 
                     output_path: Path):
...
def save_json_results(result: PipelineResult, config: CoRGiConfig,
                     timings: Dict[str, float], validation: Dict[str, Any],
                     output_path: Path, question: str):
...
def save_markdown_report(result: PipelineResult, config: CoRGiConfig,
                        timings: Dict[str, float], validation: Dict[str, Any],
                        output_path: Path, question: str):
...
def main():
    parser = argparse.ArgumentParser(
        description="Test CoRGi pipeline with real Qwen3-VL-4B-Instruct + Florence-2 models"
    )
    parser.add_argument(
        "--config",
        type=Path,
    ...
    parser.add_argument(
        "--image",
        type=str,
        help="Path to test image (default: fetch demo image)"
    )
    parser.add_argument(
        "--question",
        type=str,
        help="Question to ask (default: demo question)"
    )
    parser.add_argument(
        "--save-viz",
        action="store_true",
        help="Save bbox visualization image"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        "--disable-tracing",
        action="store_true",
        help="Disable comprehensive tracing"
    )
    parser.add_argument(
        "--trace-dir",
        type=Path,
    ...
    header = Panel(
        ...
        "[cyan]Qwen3-VL-4B-Instruct + Florence-2[/cyan]",
    ...
    console.print(f"[green]✓[/green] Config loaded from: {args.config} ({timings['Config Loading']:.2f}s)")
    ...
    console.print(f"[green]✓[/green] Pipeline completed in {timings['Pipeline Execution']:.2f}s")
    ...
    # Display results
    display_reasoning_results(result.steps)
